...
# Условный импорт WeasyPrint вместо прямого импорта
try:
...
def cart(request):
    ...
    if cart_obj:
        ...
        if cart_total_in_paise < 100:
            messages.warning(
                request,
                'Total amount in cart is less than the minimum required amount (1.00 INR). Please add a product to the cart.')
            return redirect('index')
...
# HTML to PDF Conversion - модифицированная функция с проверкой доступности WeasyPrint
def render_to_pdf(template_src, context_dict={}):
    # Если WeasyPrint недоступен, возвращаем None
...
# Модифицированная функция download_invoice с обработкой случая, когда WeasyPrint недоступен
def download_invoice(request, order_id):
    ...
    # Если WeasyPrint недоступен, показываем HTML версию счета
    if not WEASYPRINT_AVAILABLE:
        messages.warning(request, "PDF generation is not available. WeasyPrint is not installed.")
        return render(request, 'accounts/invoice.html', context)

    # Пытаемся создать PDF
    pdf = render_to_pdf('accounts/order_pdf_generate.html', context)
    if pdf:
        return pdf

    # Если что-то пошло не так с PDF, возвращаем HTML версию
    messages.error(request, "Error generating PDF. Showing HTML version instead.")
    return render(request, 'accounts/invoice.html', context)
...
# Order Details view - модифицированная версия с флагом доступности PDF
@login_required
def order_details(request, order_id):
    ...
    context = {
        ...
        'pdf_available': WEASYPRINT_AVAILABLE  # Добавляем информацию о доступности PDF
    }
